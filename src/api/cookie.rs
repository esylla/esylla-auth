...
mod oauth_state {
    ...
    /// Apply the same `__Host-`/`__Secure-` prefix rules as the session cookie.
    ...
    fn cookie(cfg: &CookieConfig, value: String) -> Cookie<'static> {
    ...
    /// Bind the OAuth `state` to the initiating browser (login-CSRF defense).
    pub(crate) fn set(jar: CookieJar, cfg: &CookieConfig, state: String) -> CookieJar {
        jar.add(cookie(cfg, state))
    }

    pub(crate) fn get(jar: &CookieJar, cfg: &CookieConfig) -> Option<String> {
        jar.get(&prefixed(cfg, OAUTH_STATE))
            .map(|c| c.value().to_owned())
    }

    pub(crate) fn clear(jar: CookieJar, cfg: &CookieConfig) -> CookieJar {
        jar.remove(cookie(cfg, String::new()))
    }
}

#[cfg(feature = "oauth")]
pub(crate) use oauth_state::{
    clear as clear_oauth_state, get as oauth_state, set as set_oauth_state,
};
